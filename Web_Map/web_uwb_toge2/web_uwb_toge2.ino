...
const char* password = "Welcome@123"; // Replace with your Wi-Fi
...
float a1_x = 4.52, a1_y = 0.0;  // Anchor-1 coordinates          .. first fix all the anchor point properly ..
float a2_x = -3.50, a2_y = 0.0;  // Anchor-2 coordinates         .. then take any Tag to (0,0) and find the ..
//                                                               .. values & value may vary slidely ..........                        
//                                                               .............................................
...
void setup() {
  ...
  server.on("/get_position", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    String response = "Tag Position: (";
    response += String(x, 4) + ", " + String(y, 4) + ")\n";  // 4 decimal places
    response += "A1 Range: " + String(a1_range, 4) + "m\n";  // 4 decimal places
    response += "A2 Range: " + String(a2_range, 4) + "m\n";  // 4 decimal places
    response += "Anchor Distance: " + String(distance_a1_a2, 4) + "m";  // 4 decimal places
  ...
  server.on("/get_ranges", HTTP_GET, [](AsyncWebServerRequest *request) {
    ...
    response += "A2 Range: " + String(a2_range, 4) + "m";  // 4 decimal places
    request->send(200, "text/plain", response);
  });
...
void loop() {
  ...
  Serial.print(x, 4);  // Print x-coordinate with 4 decimal points
  Serial.print(", ");
  Serial.print(y, 4);  // Print y-coordinate with 4 decimal points
  Serial.print(") | A1: ");
  Serial.print(a1_range, 4);  // Anchor-1 range with 4 decimal points
  Serial.print("m | A2: ");
  Serial.print(a2_range, 4);  // Anchor-2 range with 4 decimal points
  Serial.print("m | Dist_A1_A2: ");
  Serial.print(distance_a1_a2, 4);  // Distance between anchors with 4 decimal points
  Serial.println("m");
...
void readData() {
  if (mySerial.available()) {
    ...
    while (mySerial.available()) {
      ...
      response += c;
      delay(1);  // Delay to ensure proper data reading
    }
    ...
    if (response.indexOf("distance0:") != -1 && response.indexOf("distance1:") != -1) {
      ...
      a1_range = a1_range + a1_offset;
      a2_range = a2_range + a2_offset;
    }
  }
}
...
void sendATCommand(String command) {
  ...
  delay(1);  // Give time for the UWB module to process
}
...
void trilateration(float x1, float y1, float d1, float x2, float y2, float d2, float distance_a1_a2, float &x, float &y) {
  // Calculate coordinates using known distances
  float A = 2.0 * (x2 - x1);
  float B = 2.0 * (y2 - y1);
  ...
  y = y_value >= 0 ? sqrt(y_value) : 0;  // Ensure no negative square roots
}
