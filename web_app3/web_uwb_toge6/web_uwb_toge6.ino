...
#define RX_PIN 20  // RX pin connected to UWB module
#define TX_PIN 21  // TX pin connected to UWB module

// Preferences object for storing anchor data
...
void setup() {
  ...
  // Initialize Preferences for storing anchor data
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request) {
    // Show saved anchor data on the root route
    String savedData = preferences.getString("anchorData", "{}");
    request->send(200, "application/json", savedData);
  });
...
void loop() {
  ...
  delay(500);  // Delay to prevent watchdog timeout
  yield();     // Yield periodically to allow the watchdog timer to reset
}
